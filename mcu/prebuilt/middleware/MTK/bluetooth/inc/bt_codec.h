...
typedef enum {
    ...
    BT_CODEC_MEDIA_AUDIO_DL_BUF_INF_REPORT,               /**< Report share buffer informai has been updated. */
...
typedef struct _bt_media_handle_t {
    ...
    void (*set_start_asi)(struct _bt_media_handle_t *handle, uint32_t asi);                             /**< Start asi. */
    void (*set_start_bt_clk)(struct _bt_media_handle_t *handle, uint32_t bt_clk, uint32_t bt_intra_clk);/**< Start bt clk. */
} bt_media_handle_t;
...
typedef struct {
    ...
    uint32_t vendor_id;         /**< The vendor id. It contains a 32-bit vendor ID administered by the Vendor. */
} bt_codec_vendor_t;
