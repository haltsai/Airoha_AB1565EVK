...
# expand syslog buffer
AIR_SYSLOG_BUFFER_EXPAND_ENABLE       = y
AIR_SYSLOG_BUFFER_SIZE_16K_ENABLE     = y
...
AIR_OPUS_DECODER_ENABLE             = n
ifeq ($(MTK_MP3_DECODER_ENABLED)_$(MTK_WAV_DECODER_ENABLE),y_y)
    $(error "MP3 WAV OPUS can't enable at the same time")
endif
ifeq ($(MTK_MP3_DECODER_ENABLED)_$(AIR_OPUS_DECODER_ENABLE),y_y)
    $(error "MP3 WAV OPUS can't enable at the same time")
endif
ifeq ($(MTK_WAV_DECODER_ENABLE)_$(AIR_OPUS_DECODER_ENABLE),y_y)
    $(error "MP3 WAV OPUS can't enable at the same time")
endif
# record middleware
MTK_RECORD_ENABLE                   = y
...
MTK_AM_NOT_SUPPORT_STREAM_IN        = n
MTK_SBC_ENCODER_ENABLE              = n
GSOUND_HOTWORD_ENABLE               = n

ifeq ($(MTK_AWS_MCE_ENABLE), n)
GSOUND_ENABLE_SINGLE_MODE = n
endif
...
MTK_IAP2_PROFILE_ENABLE 			= n
MTK_MFI_COPROCESSOR_ENABLE 			= n
...
AMA_TRIGGER_MODE_PTT_ENABLE = n
AMA_TRIGGER_MODE_WWD_ENABLE = n
...
AIR_BT_LE_AUDIO_ENABLE                          = y
AIR_LE_AUDIO_ENABLE                             = y
AIR_LE_AUDIO_MULTIPOINT_ENABLE                  = y
...
# Xiaoai
XIAOAI_LIB = $(strip $(SOURCE_DIR))/middleware/MTK/xiaoai/module.mk
ifeq ($(XIAOAI_LIB), $(wildcard $(XIAOAI_LIB)))
MTK_VA_XIAOAI_ENABLE = y
MTK_XIAOAI_WWE_ENABLE = n
MTK_XIAOAI_PTT_ENABLE = n
endif
...
MTK_SMART_CHARGER_1WIRE_EVK = y

AIR_PMU_CHARGER_PFM_ENABLE = n

#ANC wind detection
AIR_ANC_WIND_DETECTION_ENABLE = y

#ANC User Unaware
ifeq ($(MTK_ANC_ENABLE), y)
AIR_ANC_USER_UNAWARE_CODE = $(strip $(SOURCE_DIR))/../dsp/middleware/MTK/dspalg/user_unaware/module.mk
ifeq ($(AIR_ANC_USER_UNAWARE_CODE), $(wildcard $(AIR_ANC_USER_UNAWARE_CODE)))
AIR_ANC_USER_UNAWARE_ENABLE = y
endif
endif

# UL fixed sampling rate
AIR_UL_FIX_SAMPLING_RATE_48K = n

# Auido advanced passthrough feature enable
AIR_ADVANCED_PASSTHROUGH_LIB = $(strip $(SOURCE_DIR))/prebuilt/middleware/MTK/psap/module.mk
ifeq ($(AIR_ADVANCED_PASSTHROUGH_LIB), $(wildcard $(AIR_ADVANCED_PASSTHROUGH_LIB)))
AIR_ADVANCED_PASSTHROUGH_ENABLE = y
AIR_ADVANCED_PASSTHROUGH_SOURCE_CODE = $(strip $(SOURCE_DIR))/../mcu/middleware/MTK/audio/psap_protected/GCC/module.mk
ifeq ($(AIR_ADVANCED_PASSTHROUGH_SOURCE_CODE), $(wildcard $(AIR_ADVANCED_PASSTHROUGH_SOURCE_CODE)))
...
#ANC Environment_Detection
ifeq ($(MTK_ANC_ENABLE), y)
AIR_ANC_ENVIRONMENT_DETECTION_CODE = $(strip $(SOURCE_DIR))/../dsp/middleware/MTK/dspalg/env_detect/module.mk
...
AIR_SILENCE_DETECTION_ENABLE = n
