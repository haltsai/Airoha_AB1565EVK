...
//for weakup sleep
static uint8_t syslog_uart_rx_sleep_handle = 0xFF;
static uint32_t syslog_sleep_lock_timer = NULL;
static bool syslog_get_handle = false;
#define RX_TIME_OUT_MS 10000
...
void syslog_lock_sleep_callback(void *user_data)
{
    if (hal_sleep_manager_is_sleep_handle_alive(syslog_uart_rx_sleep_handle) == true) {
        hal_sleep_manager_unlock_sleep(syslog_uart_rx_sleep_handle);
    }

    hal_gpt_sw_stop_timer_ms(syslog_sleep_lock_timer);
    hal_gpt_sw_free_timer(syslog_sleep_lock_timer);
...
static void user_uart_callback(hal_uart_callback_event_t event, void *user_data)
{
    ...
    if (event == HAL_UART_EVENT_READY_TO_READ) {
        ...
        if (syslog_get_handle == false) {
            syslog_uart_rx_sleep_handle = hal_sleep_manager_set_sleep_handle("syslog");
            syslog_get_handle = true;
        }
        if (HAL_GPT_STATUS_OK != hal_gpt_sw_get_timer(&syslog_sleep_lock_timer)) {
            return;
        }
        if (HAL_GPT_STATUS_OK != hal_gpt_sw_start_timer_ms(syslog_sleep_lock_timer, RX_TIME_OUT_MS, syslog_lock_sleep_callback, NULL)) {
            hal_gpt_sw_free_timer(syslog_sleep_lock_timer);
            return;
        }
        if (hal_sleep_manager_is_sleep_handle_alive(syslog_uart_rx_sleep_handle) == false) {
            hal_sleep_manager_lock_sleep(syslog_uart_rx_sleep_handle);
        }
    }
}
