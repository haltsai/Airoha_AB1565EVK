...
exception_status_t exception_dsp_dump_preprocess(int num, exception_sharemem_dsp_t *pESM)
{
    ...
    else
    {
        /* CM4 need to transfer DSP1 address into CM4 view */
        context_regs = (exception_context_dsp_t *)hal_memview_dsp1_to_cm4((uint32_t)(pESM->context_regs));
        dsp_assert_info  = (assert_expr_t *)hal_memview_dsp1_to_cm4((uint32_t)(pESM->assert_info));
    }
    ...
    if (dsp_assert_info->is_valid == 1)
    {
        ...
        else
        {
            ...
            exception_printf("DSP%d assert failed: %s, file: %s, line: %d\r\n",
                             pESM->corenum,
                             (const char *)hal_memview_dsp1_to_cm4((uint32_t)(dsp_assert_info->expr)),
                             (const char *)hal_memview_dsp1_to_cm4((uint32_t)(dsp_assert_info->file)),
                             dsp_assert_info->line);
        }
    }
    else if (dsp_assert_info->is_valid == 2)
    {
        if(num == 0)
        {
            exception_msgid(EXCEPTION_CPU_ID_SLAVE0,
        ...
        {
            exception_msgid(EXCEPTION_CPU_ID_SLAVE1,
                            dsp_assert_info->string,
                            4,
                            pESM->corenum,
                            dsp_assert_info->expr,
    ...
    /* Check if there is a assert */
    if (dsp_assert_info->is_valid)
    {
        minidump_header_dsp.assert_info.is_valid = dsp_assert_info->is_valid;
        if(num == 0)
        {
            /* CM4 need to transfer DSP0 address into CM4 view */
            minidump_header_dsp.assert_info.expr = (const char *)hal_memview_dsp0_to_cm4((uint32_t)(dsp_assert_info->expr));
            minidump_header_dsp.assert_info.file = (const char *)hal_memview_dsp0_to_cm4((uint32_t)(dsp_assert_info->file));
        }
        else
        {
            /* CM4 need to transfer DSP1 address into CM4 view */
            minidump_header_dsp.assert_info.expr = (const char *)hal_memview_dsp1_to_cm4((uint32_t)(dsp_assert_info->expr));
            minidump_header_dsp.assert_info.file = (const char *)hal_memview_dsp1_to_cm4((uint32_t)(dsp_assert_info->file));
        }
        minidump_header_dsp.assert_info.line = dsp_assert_info->line;
    }
    else
    {
...
void exception_dsp_dump_postprocess(int num, exception_sharemem_dsp_t *pESM)
{
    ...
    if(minidump_base_address != 0)
    {
        ...
        if(num == 0)
        {
            ...
            /* feed wdt to keep time for saving header */
            exception_feed_wdt();
...
{
    if(hal_core_status_read(HAL_CORE_DSP1) > HAL_CORE_OFF)
    {
        exception_enable_wdt_interrupt();
    }
}
